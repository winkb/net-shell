...
impl TemplateEngine {
    ...
    pub fn with_all_delimiters(var_left: &str, var_right: &str, for_left: &str, for_right: &str) -> Self {
        ...
        // 变量匹配正则：{{ variable }}
        let var_pattern = format!(r"{}\s*([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)\s*{}", var_left_escaped, var_right_escaped);
        ...
        let include_pattern = format!("{}\\s*include\\s+\"([^\"]+)\"\\s*{}", for_left_escaped, for_right_escaped);
    ...
    /// 设置变量
    pub fn set_variable<K: Into<String>, V: Into<serde_json::Value>>(&mut self, key: K, value: V) -> &mut Self {
    ...
    fn process_includes(&self, template: &str) -> Result<String> {
        ...
        while let Some(captures) = self.include_regex.captures(&result) {
            ...
            let included_content = if let Some(ref dir) = self.template_dir {
                ...
                return Err(anyhow!("Template directory not set for include: {}", template_name));
            };
    ...
    fn process_for_loops(&self, template: &str) -> Result<String> {
        ...
        while let Some(captures) = self.for_regex.captures(&result) {
            ...
            let array_value = self.variables.get(array_name)
            ...
            if let serde_json::Value::Array(items) = array_value {
                ...
                for item in items {
                    ...
                    let rendered = temp_engine.process_variables(loop_content)?;
    ...
    fn get_variable_value(&self, path: &str) -> Result<serde_json::Value> {
        ...
        let current = self.variables.get(parts[0])
        ...
        for part in &parts[1..] {
            match result {
                ...
                _ => return Err(anyhow!("Cannot access property '{}' on non-object value", part)),
            }
        }
...
mod tests {
    ...
    fn test_variable_replacement() {
        ...
        let result = engine.render_string("Hello, {{ name }}! You are {{ age }} years old.").unwrap();
    ...
    fn test_nested_variable_access() {
        ...
        let result = engine.render_string("Name: {{ user.name }}, City: {{ user.profile.city }}").unwrap();
